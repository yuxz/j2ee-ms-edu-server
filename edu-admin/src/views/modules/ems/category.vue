<template>
  <div>
    <el-switch
      ...
      inactive-text="关闭拖拽"
    ></el-switch>
	<el-tooltip content="新增一级份分类" placement="top">
    <el-button
      type="primary"
      @click="() => append()"
      icon="el-icon-plus"
...
export default {
  ...
  methods: {
    ...
    batchSave() {
      this.$http({
        url: this.$http.adornUrl("/exam/category/update/sort"),
        method: "post",
    ...
    edit(data) {
      ...
      //发送请求获取当前节点最新的数据
      this.$http({
        url: this.$http.adornUrl(`/exam/category/info/${data.id}`),
        method: "get",
      }).then(({ data }) => {
        //请求成功
    ...
    editCategory() {
      ...
      this.$refs["categoryForm"].validate((valid) => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl("/exam/category/update"),
            method: "post",
    ...
    addCategory() {
      ...
      this.$refs["categoryForm"].validate((valid) => {
        if (valid) {
          //alert('submit!');
          this.$http({
            url: this.$http.adornUrl("/exam/category/save"),
            method: "post",
