      ...
      >
        <template slot-scope="scope">
          <el-switch
            ...
            :active-value="0"
            :inactive-value="1"
            @change="changeIsfihished(scope.row)"
          >
          </el-switch>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 结业
    changeIsfihished(data) {
	  let { id, isFinished } = data;
	  console.log("结业"+ data);
      this.$http({
        url: this.$http.adornUrl("/ims/class/update"),
        method: "post",
        data: this.$http.adornData({ id, isFinished }, false),
      }).then(({ data }) => {
        this.$message({
          type: "success",
          message: "更新成功",
