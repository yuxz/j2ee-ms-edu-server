...
<script>
  ...
  export default {
    ...
    methods: {
	  ...
	  getChartLineData(){	
		this.$http({
			url: this.$http.adornUrl("/ims/statistics/line"),
			method: "get",
			}).then(({ data }) => {
				console.log("成功获取Chartline数据...", data.data);
	  ...
	  setChartPie(){
		var option = {
          ...
          series: [
            {
              name: this.chartPieData.series[0].name,
              type: 'pie',
              radius: '55%',
              center: ['50%', '50%'],
              data: this.chartPieData.series[0].data.sort(function (a, b) { return a.value - b.value }),
              roseType: 'radius',
              label: {
                normal: {
                  textStyle: {
      ...
      initChartScatter () {
		...
		// 		this.charScatterData = data.data;
		// });
