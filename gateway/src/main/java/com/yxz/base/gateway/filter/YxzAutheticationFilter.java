package com.yxz.base.gateway.filter;
...
import com.yxz.base.common.utils.JwtUtils;
...
public class YxzAutheticationFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if(strings != null && strings.size()>0){
            ...
            try {
                //验证令牌通过，放行了
                Map<String, Object> body = JwtUtils.getJwtBody(s);
                //自动进行redis的数据续期
                //{"id":1,"token":"b7c1ce9e434a4827b495c191e943aec6"}
                String token = (String) body.get("token");
                String key = IamServerConstant.LOGIN_USER_PREFIX+token;
                //自动续期
